...
const infoPanelData = [
  {
    ...
    description:
      "Motive a sua equipe com rankings em tempo real e desafios estimulantes.",
  ...
  },
  {
    ...
    description:
      "Acompanhe a sua performance e evolução com gráficos claros e objetivos.",
  ...
  },
  {
    ...
    description:
      "Defina metas claras e celebre as conquistas com um sistema de recompensas.",
  ...
  return (
    <div className="absolute top-0 left-0 h-full w-1/2 flex items-center justify-center p-12 text-center backdrop-blur-lg bg-white/10 rounded-l-2xl">
      <div className="w-full max-w-sm">
        ...
        <h1 className="mb-6 text-3xl font-normal tracking-wider text-white">
          Fazer Login
        </h1>
          ...
          <div>
            <div className="relative">
              ...
              <button
                ...
                disabled={isLoading}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-white disabled:opacity-50"
              >
                {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
              </button>
            </div>
            {errors.password && (
              <p className="text-sm text-red-400 mt-1 text-left">{errors.password.message}</p>
            )}
          </div>
          <a
            href="#"
            className="block text-right text-xs text-gray-300 hover:underline"
          >
...
const InfoPanel = () => {
  return (
    <div className="absolute top-0 right-0 h-full w-1/2 flex flex-col items-center justify-center p-12 text-center text-white backdrop-blur-lg bg-white/10 rounded-r-2xl">
      <h1 className="mb-8 text-3xl font-normal tracking-wider">
      ...
      <div className="space-y-6 text-left w-full">
        {infoPanelData.map((item) => (
          <div key={item.title} className="flex items-start space-x-4">
            <item.icon
              className={`h-8 w-8 ${item.color} mt-1 flex-shrink-0`}
            />
            <div>
          ...
          <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-[#3f035d] to-[#1a0e18] p-12 text-center text-white">
            {!isPanelActive && (
              <div className="transition-opacity duration-300 ease-in-out opacity-100 delay-200">
                <h1 className="mb-4 text-3xl font-normal tracking-wider">
                  Ainda não faz parte?
                </h1>
                <p className="mb-6 text-gray-200">
                  Clique para saber mais sobre a plataforma que vai
                  transformar a sua performance.
                </p>
                <button
                  type="button"
            ...
            {isPanelActive && (
              <div className="transition-opacity duration-300 ease-in-out opacity-100 delay-200">
                ...
                <p className="mb-6 text-gray-200">
                  Faça o login para aceder ao seu dashboard e começar a
                  competir.
                </p>
                <button
                  type="button"
